 ...
 */
exports.getPaginatedPayload = (dataArray, page, limit) => {
  ...
  const payload = {
    ...
    nextPage: dataArray.length === limit && structuredClone(dataArray).pop()?.id < totalItems ? true : false,
    totalItems,
...
exports.removeUnusedMulterImageFilesOnError = (req) => {
  try {
    ...
    if (multerFiles) {
      ...
      console.log(filesValueArray, "filesValueArray");
      filesValueArray.map((fileFields) => {
        console.log(fileFields, "fileFields");
        this.removeLocalFile(fileFields.path);
 ...
 */
exports.getMongoosePaginationOptions = ({ page = 1, limit = 10, customLabels }) => {
  return {
